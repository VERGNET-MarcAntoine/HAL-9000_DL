...
fig, ax = plt.subplots()
ax.set_xlim(-10000 , 10000)  # Ajuste selon la taille de ton univers
...
def update(frame):
    ...
    ship_positions = np.array([ship["body"]["position"][0:2] for ship in ships])
    planet_positions = np.array([planet[1][0:2] for planet in planets[1:-1]])  # Vérifie que les données sont bien en (x, y)
